    ...
    <footer className='max-conatiner'>
      <div className='flex justify-between items-start gap-20 flex-wrap max-lg:flex-col'>
        <div className='flex flex-col items-start '>
          ...
          < p className='text-base text-white-400 mt-6 leading-7 font-montserrat sm:max-w-sm'>Get shoes ready for the new
        ...
        <div className="flex flex-1 justify-between lg:gap-10 gap-20 flex-wrap">
          {footerLinks.map((section) => (
            <div key={section}>
              <h4 className='leading-normal mb-6 font-medium text-white font-montserrat text-2xl'>{section.title}</h4>
              <ul>
                {section.links.map((link) => (
                  <li className="mt-3 text-white-400 font-montserrat text-base leading-normal hover:text-slate-gray cursor-pointer"
                    key={link.name}><a>{link.name}</a></li>
                ))}
              </ul>
            </div>
          ))}
